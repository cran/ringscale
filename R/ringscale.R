...
evaluate <- function(filename = character(0), support.min = 0.5) {
	...
	# this data will be loaded from the file in the next step
	hypotheses.combined <- NULL
	...
	load(filename)
	if (is.null(hypotheses.combined)) {
...
analyze <- function (filenames = character(0), result.filename = paste(format(Sys.time(), "%Y-%m-%d-%H%M%S"), ".RData", sep = ""), evaluate = TRUE, header.skip = FALSE) {
	...
	if (!part.continued) {
		...
		# or skip if user wants to (probably because of performance reasons)
		if (!header.skip) {
	...
	while (any(!(hypotheses.combined$info$files))) {
		...
		if (all(hypotheses.combined$info$files)) {
			if (file.exists(paste(hypotheses.combined$info$result.filename, ".ringscale.part", sep = ""))) {
